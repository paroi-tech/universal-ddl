import { AstColumn, AstColumnConstraintComposition, AstCreateTable } from "../src/ast"
  ...
  for (const dataType of simpleTypes) {
    test(`data type: ${dataType}`, () => {
      const input = `
        ...
        `
      const table = parseDdl(input).orders[0] as AstCreateTable
